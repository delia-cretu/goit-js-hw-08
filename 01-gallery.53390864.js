...
(0, _galleryItems.galleryItems).forEach((galleryItem)=>{
    gallery.insertAdjacentHTML("afterbegin", `<li class="gallery__item"><a class="gallery__link" href="${galleryItem.original}"><img class="gallery__image" src = "${galleryItem.preview}" alt = "${galleryItem.description}"></img></a></li>`);
});
// Simple Lightbox
var lightbox = new (0, _simplelightboxDefault.default)(".gallery__link", {
    captionsData: "alt",
    captionDelay: "250"
});
gallery.addEventListener("click", openImageInLightbox);
function openImageInLightbox(event) {
    if (event.target.getAttribute("class") !== "gallery__link") return;
